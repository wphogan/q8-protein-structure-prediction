...
def train(epochs, model, stats_path,
          ...
          len_train, len_val,
          latest_model_path,
          best_model_path, optim_path, device, early_stop=10, num_features=51):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        accs, val_loss = val(epoch, model, val_loader, len_val, criterion, epochs, device, num_features)
...
def val(epoch, model, val_loader, len_val, criterion, epochs, device, num_features=51):
...
def test(model, test_loader, device, num_features=51, experiment="Name of experiment"):
    all_labels = []
    all_predictions = []
    model.eval()
